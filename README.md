...
We present an adaption of the Social Force Model from Helbing et al. [2] to model speaker movement. Via a Neutonian formulation, we achieve smooth and nnvironmentally aware speaker trajectories in enclosed acoustic scenarios. This repository provides the code that generates the dataset used in our work in [1], which uses speech mixtures based on LibriMix [3] with additonal diffuse noise according to the recipe in [4].
...
[1] J. Kienegger, T. Gerkmann, "Autoregressive Guidance of Deep Spatially Selective Filters using Bayesian Tracking for Efficient Extraction of Moving Speakers", *Preprint*, 2026. [arXiv]
